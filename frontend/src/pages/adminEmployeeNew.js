...
function AdminEmployeeNew() {
    ...
    const onInputUN = ({ target: { value } }) => {
        console.log(value);
        if (bigList[2].some(item => item.username === value)) {
            setAlertMsg("Username already used!");
            setShowAlert(true);
        } else {
            setAlertMsg("");
            setShowAlert(false);
    ...
    const onInputET = ({ target: { value } }) => {
        setEmployeeType(value);
        switch (value) {
            case 'Administrative':
                setAnnualLeaveEntitlement(14);
                formRef.current.querySelector("#formAnnual").value = 14;
                break;
            case 'Professional':
                setAnnualLeaveEntitlement(18);
                formRef.current.querySelector("#formAnnual").value = 18;
                break;
        }
    }
        ...
        if (getUserinfoFromLocal()) {
            getAllList()
                ...
                .then((list) => {
                    console.log(list);
                    setBigList(list);
                    setMedicalLeaveEntitlement(60);
                });
        }
    };

    const resetFormFields = () => {
        formRef.current.querySelector("#formUserName").value = '';
        formRef.current.querySelector("#formPassword").value = '';
    };
    ...
    const onFormSubmit = (event) => {
        ...
        else if (bigList[2].some(item => item.username === formRef.current.querySelector("#formUserName").value)) {
            event.stopPropagation();
            setAlertMsg("Username already used!");
            setShowAlert(true);
        }
