...
public class AdminController {
	...
	@GetMapping(value = { "/employee", "/employee/list" })
	public String viewEmployeeList(HttpSession session, Model model) {
		// currentAdmin cannot delete himself, so we add currentID to the model
		LAPSUser currentAdmin = (LAPSUser) session.getAttribute("user");
		model.addAttribute("currentId", currentAdmin.getUserId());
	...
	public String deleteEmployee(@PathVariable int id) {
		// delete all leave applications before delete user
	...
	public String newEmployee(Model model) {
		...
		List<LAPSUser> managers = userservice.findAllManager();
	...
	public String createEmployee(@ModelAttribute LAPSUser newEmployee, @RequestParam String roleId) {
		newEmployee.setRole(roleservice.findRoleByName(roleId));
